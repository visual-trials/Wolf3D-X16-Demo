...
; FIXME: turn this off!
    jsr test_speed_of_clearing_screen
stop_now:
    jmp stop_now
...
test_speed_of_clearing_screen:
    ...
    jsr tmp_generate_clear_column_code

    jsr start_timer

; FIXME: use a fast clear screen method!
;    jsr clear_bitmap_screen
    jsr tmp_clear_screen_fast
    ...
    lda #<tmp_clear_screen_320x240_8bpp_message
    sta TEXT_TO_PRINT
    lda #>tmp_clear_screen_320x240_8bpp_message
    sta TEXT_TO_PRINT + 1
    ...
    lda #<tmp_clear_screen_1_byte_message
    sta TEXT_TO_PRINT
    lda #>tmp_clear_screen_1_byte_message
    sta TEXT_TO_PRINT + 1
    ...
    jsr tmp_print_time_elapsed

    rts
    
tmp_print_time_elapsed:

    lda #<tmp_time_elapsed_message
    sta TEXT_TO_PRINT
    lda #>tmp_time_elapsed_message
    sta TEXT_TO_PRINT + 1
    ...
    lda #<tmp_time_elapsed_ms_message
    sta TEXT_TO_PRINT
    lda #>tmp_time_elapsed_ms_message
    sta TEXT_TO_PRINT + 1
...
tmp_clear_screen_320x240_8bpp_message: 
    .asciiz "Cleared screen 320x240 (8bpp) "
tmp_clear_screen_1_byte_message: 
    .asciiz "Method: 4 bytes per write"
tmp_time_elapsed_message: 
    .asciiz "Elapsed time... "
tmp_time_elapsed_ms_message: 
    .asciiz " ms  "

    
    
tmp_clear_screen_fast:

    ; Left part of the screen (256 columns)

    ldx #0
    
tmp_clear_next_column_left:
    ...
    inx
    inx
    bne tmp_clear_next_column_left
    
    ; Right part of the screen (64 columns)

    ldx #0

tmp_clear_next_column_right:
    lda #%11100110           ; Setting bit 16 of vram address to the highest bit (=0), setting auto-increment value to 320 bytes (=14=%1110)
    sta VERA_ADDR_BANK
    lda #$01
    sta VERA_ADDR_HIGH
    ...
    inx
    cpx #64
    bne tmp_clear_next_column_right
    
    rts
    
    
tmp_generate_clear_column_code:
...
; FIXME: this will overflow into KEYBOARD_SCANCODE_BUFFER!!
    ...
    bne tmp_next_clear_instruction
